...
class NotificationCenter {
    ...
    addNotes(notes) {
        notes.forEach((item, i) => {
            let existingItem = this.items.find(function(e) {
                return (e.args.title === item.title && e.args.icon === item.icon && e.args.subtitle === item.subtitle && e.args.actions.length === item.actions.length)
            });
            let exisingElement;
            if (existingItem && (exisingElement = $('#' + existingItem.id + ' .notification__box'))) {
                exisingElement.css({'animation': 'pulse 2s infinite'});
                setTimeout(function () {
                    exisingElement.css({'animation': 'none'});
                },6 * 1000);
                return false;
            } else {
    ...
    spawnNote() {
        ...
        const draw = this.messages.length - 1;
        ...
        if (message) {
            ...
            this.killNote(this, note.id);
            this.messages.splice(draw, 1);
        }
    }

    spawnNotes(amount) {
        let count = typeof amount === "number" ? amount : this.messages.length;
        while (count--) {
            this.spawnNote();
        }
    }
...
class Notification {
    ...
    init(args) {
        ...
        actions.forEach(action => {
            ...
            if (isObject(action) && Object.hasOwn(action, 'title') && Object.hasOwn(action, 'url')) {
                btn.setAttribute("data-href", action.url);
                btnText.innerHTML = action.title;
            } else {
                btnText.innerHTML = action;
            }
...
$(document).on('click', '.notification__btn[data-href]', function () {
    console.log('success');
...
function NotificationMessages() {
    return [
        ...
        {
            icon: "success",
            title: "Table was updated",
            subtitle: "Table was successfully updated. Reload the page to see updates.",
            actions: ["OK"]
        },
        {
            icon: "success",
...
$(document).ready('click', function () {
    notifier.addNotes([{
        icon: "success",
        title: "Table was successfully updated",
        actions: ["OK", {title: 'Refresh page', url: '/admin'}]
    }]);
    notifier.spawnNote(1);
});